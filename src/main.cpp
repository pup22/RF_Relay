...
void copy_array_from_eeprom() { //  перенос записей в RAM
  for(int i = 0; i < count_addr; i++) {
    array_addr[i] = eeprom_read_dword(EEPROM_ADDR + i * 4 + 1);
  }
}

bool check_uniq_code (uint32_t code) {  // проверка на уникальность
  for(int i = 0; i < count_addr; i++) {
    if (array_addr[i] == code) return false;
  }
  return true;
}

void doubleclick() {  //запись нового пульта
  ...
  if (count_addr >= COUNT_MAX) {  // превышено кол-во записей
  ...
  delay(500);
  for(int i = 0; i < count_addr; i++) { // мигаем кол-вом записей
    digitalWrite(LED, HIGH);
    delay(250);
    digitalWrite(LED, LOW);
    delay(250);
  ...
  while ((millis() - t) < 5000) //  ждем новый код несколько сек.
  {
    if (mySwitch.available()) {
      ...
      if(check_uniq_code(code)) { // если код уникальный записываем
...
void setup() {
  ...
  if(!digitalRead(BUTTON)) {  // если при старте нажата кнопка больше секунды - стираем все коды
    delay(2000);
    if(!digitalRead(BUTTON)) {
      eeprom_write_byte(EEPROM_ADDR, 255);
      //Serial.println("Erase");
      count_addr = 0;
      for (int i = 0; i < 50; i++) {  // помигаем "типа память очищенна"
        digitalWrite(LED, !digitalRead(LED));
        delay(80);
      }
    }
